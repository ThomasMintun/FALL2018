...
//  L    D    =    <    >    :    +    -    *    /   %    .    (    )    ,    {    }    ;    [    ]    WS  EOF  ERROR
 ...
 {  1,   1, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500}, // 1 IDENTIFIER Token
 {501,   2, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501, 501}, // 2 NUMBER Token
 {502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502, 502}, // 3 =Token
 {503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503, 503}, // 4 <Token
 {504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504, 504}, // 5 >Token
...
char_m specialChars[] = {//{left value is ascii value, right value is column} ARRAY SUBSCRIPTS START AT 0!
	...
	{45, 7}, {42, 8}, {47, 9}, {37, 10}, {46, 11},		// -, *, /, %, .
	{40, 12}, {41, 13}, {44, 14}, {123, 15},	// (, ), ,, {
	{125, 16}, {59, 17}, {91, 18}, {93, 19}     // }, ;, [, ], 
};
...
token_t lookup(int state, std::string searchString) {
	...
	if (state == 500){
		for (i = 0; i < KEYWORDS; i++) {
			//std::cout << "KW: " << keywordLookupTable[i].tkString << std::endl; //for testing
			//std::cout << "SS: " << searchString << std::endl; //for testing
			if (keywordLookupTable[i].tkString == searchString){
	...
	while (currentState < 500 && currentState > -1) {
		...
		if (nextState > 499 || nextState == -1){
			...
			if (nextState == -1) {
				...
				//std::cout << "CW: " << currentWord << std::endl; //for testing
		...
		//Final state not yet reached. continue building token
			...
			if(!isspace(currentChar)){
				currentWord += currentChar;//if not WS add to word
			}
